...
import { IMM_QUESTIONS as DEFAULT_IMM_QUESTIONS } from '../../constants/imm-questions';
import { Question } from '../../types';
import { useDashboard } from '../../context/DashboardContext';

const STORAGE_PREFIX = 'donga_questions_';

const loadQuestions = (projectId: string, type: 'standard' | 'imm'): Question[] => {
  try {
    const saved = localStorage.getItem(STORAGE_PREFIX + projectId);
    if (saved) return JSON.parse(saved);
  } catch { /* ignore */ }
  return type === 'imm' ? DEFAULT_IMM_QUESTIONS : DEFAULT_QUESTIONS;
};

const saveQuestions = (projectId: string, questions: Question[]) => {
  localStorage.setItem(STORAGE_PREFIX + projectId, JSON.stringify(questions));
};

const getDefaults = (type: 'standard' | 'imm') =>
  type === 'imm' ? DEFAULT_IMM_QUESTIONS : DEFAULT_QUESTIONS;
...
export const QuestionsPage = () => {
  ...
  const [questions, setQuestions] = useState<Question[]>(() =>
    selectedProject ? loadQuestions(selectedProjectId, selectedProject.type) : []
  );

  // Reload questions when project changes
  useEffect(() => {
    if (selectedProject) {
      setQuestions(loadQuestions(selectedProjectId, selectedProject.type));
    } else {
      setQuestions([]);
    }
    setSelectedQ(null);
  ...
  useEffect(() => {
    if (selectedProjectId) {
      saveQuestions(selectedProjectId, questions);
    }
  ...
  const resetToDefault = () => {
    ...
    if (window.confirm(`"${selectedProject.name}" 문항을 기본값으로 초기화하시겠습니까?`)) {
      setQuestions(getDefaults(selectedProject.type));
      setEditMode(false);
      setSelectedQ(null);
    }
  };
  ...
  return (
    <div>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          <button
            ...
            style={{
              ...
              fontSize: 12, cursor: selectedProject ? 'pointer' : 'default', background: 'white',
              color: BRAND.gray[600], opacity: selectedProject ? 1 : 0.4,
            }}
          >
            기본값 초기화
          </button>
      ...
      <p style={{ fontSize: 13, color: BRAND.gray[500], marginBottom: 24 }}>프로젝트별 진단 문항을 확인하고 수정합니다.</p>
      ...
      <div style={{
        ...
        <span style={{ fontSize: 13, fontWeight: 600, color: BRAND.gray[600], whiteSpace: 'nowrap' }}>프로젝트</span>
        <select
          value={selectedProjectId}
          onChange={e => setSelectedProjectId(e.target.value)}
          style={{
            padding: '8px 14px', border: `1px solid ${BRAND.gray[300]}`, borderRadius: 8,
            fontSize: 13, background: 'white', cursor: 'pointer', flex: 1, maxWidth: 300,
          }}
        >
          {projects.map(p => (
            <option key={p.id} value={p.id}>
              {p.name} ({p.type === 'imm' ? 'IMM' : '표준'})
            </option>
          ))}
        </select>
        {selectedProject && (
          <div style={{ display: 'flex', gap: 12, marginLeft: 'auto' }}>
            <span style={{ fontSize: 12, color: BRAND.gray[500] }}>
              PART 1: {part1Count}문항
            </span>
            <span style={{ fontSize: 12, color: BRAND.gray[500] }}>
              PART 2: {part2Count}문항
            </span>
          ...
          <div style={{ background: 'white', borderRadius: 12, border: `1px solid ${BRAND.gray[200]}`, overflow: 'hidden' }}>
            <div style={{
              ...
              <span style={{
                ...
                background: selectedProject.type === 'imm' ? '#FEF3C7' : BRAND.gray[100],
                color: selectedProject.type === 'imm' ? '#D97706' : BRAND.gray[600],
              }}>
                {selectedProject.type === 'imm' ? 'IMM' : '표준'}
              </span>
            </div>
